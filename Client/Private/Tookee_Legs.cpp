#include "Tookee_Legs.h"
#include "GameInstance.h"
#include "TopTookee.h"

CTookee_Legs::CTookee_Legs(LPDIRECT3DDEVICE9 pGraphic_Device) :CTookee_Parts(pGraphic_Device)
{
}

CTookee_Legs::CTookee_Legs(const CTookee_Legs& Prototype) :CTookee_Parts(Prototype)
{
}

HRESULT CTookee_Legs::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTookee_Legs::Initialize(void* pArg)
{
...
void CTookee_Legs::Priority_Update(_float fTimeDelta)
{
}

void CTookee_Legs::Update(_float fTimeDelta)
{
	auto eState = m_pParent->Get_State();
	auto _vPos = m_pTransformCom->Get_State(STATE::POSITION);

	if (eState == CTopTookee::ETOPDEESTATE::WALK || eState == CTopTookee::ETOPDEESTATE::SIDE_WALK)
	{
		if (m_iAnimDir == -1)
		{
...
void CTookee_Legs::Late_Update(_float fTimeDelta)
{
	m_pGameInstance->Add_RenderGroup(RENDERGROUP::PRIORITY, this);
}

HRESULT CTookee_Legs::Render()
{
...
CTookee_Legs* CTookee_Legs::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CTookee_Legs* pInstance = new CTookee_Legs(pGraphic_Device);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CTookee_Legs");
...
CGameObject* CTookee_Legs::Clone(void* pArg)
{
	CTookee_Legs* pInstance = new CTookee_Legs(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CTookee_Legs");
		Safe_Release(pInstance);
	}
...
HRESULT CTookee_Legs::Ready_Components()
{
	...
	/* Com_Texture */
	if (FAILED(__super::Add_Component(ENUM_TO_UINT(LEVEL::STATIC), TEXT("Prototype_Component_Texture_TookeeLegs"),
...
void CTookee_Legs::SetParent(CTopTookee* ppGameObject)
{
